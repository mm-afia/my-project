      ...
      <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300">
        ...
        <div class="p-4">
          ...
          <div class="flex flex-wrap gap-2">
            ${petpal.services.slice(0, 3).map(s => `<span class="px-2 py-1 bg-primary-50 text-primary-700 text-xs rounded-full">${s}</span>`).join('')}
            ${petpal.services.length > 3 ? `<span class="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded-full">+${petpal.services.length - 3} more</span>` : ''}
          </div>
        </div>
      </div>
    </a>
...
function filterAndSort() {
  const filtered = petpals.filter(p => 
  ...
  if (container) {
    container.innerHTML = sorted.map(p => renderPetpalCard(p)).join('')
  }
  if (countEl) {
